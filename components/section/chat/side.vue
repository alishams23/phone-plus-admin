<template>
  ...
  <v-locale-provider rtl>
    <v-text-field v-model="searchInput" @input="ListUserMessageApi"
      class="shadow-none ps-5 pl-md-4 pt-0 mt-10 border-0 text-right" bg-color="grey-lighten-3" base-color="primary"
      label=" جستجو" rounded="pill" variant="solo" elevation="0">
  ...
  <v-list subheader class="pl-0 pl-md-4">

    <div v-for="person, index in contacts">
      <v-list-item v-if="person.contact"
        :to="'/chat/' + person.contact.username + '/' + person.room_name + '/' + person.contact.full_name"
        @click="$emit('get-selected-user', person); selected_user = person.contact.username"
        class="py-1 d-flex justify-start rtl ms-3 rounded-lg my-1">
